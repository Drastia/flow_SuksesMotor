...
class _CalendarPageState extends State<CalendarPage> {
  ...
  Future<void> _fetchEvents() async {
    try {
      final response = await http.get(Uri.parse(baseURL + 'orders/countOrdersByDate'));
      if (response.statusCode == 200) {
        ...
        events.forEach((event) {
          DateTime dateWithoutTime = DateTime(event.date.year, event.date.month, event.date.day);
  ...
  Future<void> _fetchOrdersForSelectedDay(DateTime selectedDay) async {
    try {
      ...
      final response = await http.get(Uri.parse(baseURL + 'orders/getOrdersByDate/$formattedDate'));
      if (response.statusCode == 200) {
        Iterable data = json.decode(response.body);
        ...
        throw Exception('Failed to load orders for selected day: ${response.statusCode}');
      }
    } catch (e) {
            ...
            calendarBuilders: CalendarBuilders(
              markerBuilder: (context, day, events) {
                ...
                if (eventList.isNotEmpty) {
                  int totalEvents = eventList.fold(0, (sum, event) => sum + event.count);
                  return Positioned(
                    right: 1,
                    bottom: 1,
                    child: Container(
                      ...
                      child: Container(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              Text('Tanggal Pemesanan: ${DateFormat('yyyy-MM-dd').format(order.tanggalPemesanan)}'),
                              Text('Tanggal Sampai: ${DateFormat('yyyy-MM-dd').format(order.tanggalSampai)}'),
                              Text('Nama Vendor: ${order.namaVendor}'),
                              Text('Nama Pemesan: ${order.namaPemesan}'),
                              Text('Checked: ${order.checked}'),
                            ],
                          ),
                        ),
                      ),
